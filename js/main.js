$(document).ready(function () {
  ...
  $('a[href^="#"]').click(function () {
    ...
    // Если элемента нет, разрешаем обычное поведение ссылки
  });
  ...
  $(".zakazat_zvonok").on("click", function () {
    $(".shadow").addClass("openedPopup");
    $(".popup.form").addClass("openedPopup");
    $('.popup.form input[type="submit"]').val(
  ...
  var before_tri_grey_seo_width =
    parseInt($(".before_tri_grey_seo").width()) / 2;
  var before_tri_grey_seo_height =
    parseInt($(".before_tri_grey_seo").height()) / 2;
  ...
  var after_tri_white_seo_width =
    parseInt($(".after_tri_white_seo").width()) / 2;
  var after_tri_white_seo_height =
    parseInt($(".after_tri_white_seo").height()) / 2;
...
function scroll_active() {
  ...
  let sections = [
    { id: "#podbor", element: $("#podbor") },
    { id: "#benefit", element: $("#benefit") },
    { id: "#about", element: $("#about") },
    { id: "#seo", element: $("#seo") },
    { id: "#work", element: $("#work") },
    { id: "#reviews", element: $("#reviews") },
    { id: "#question", element: $("#question") },
  ];

  sections = sections.filter((section) => section.element.length);

  $(".main_menu li a, .menu_f a").removeClass("active");

  for (let i = sections.length - 1; i >= 0; i--) {
    let sectionOffset = sections[i].element.offset().top - 100;
    if (window_top >= sectionOffset) {
  ...
  if (
    sections.length > 0 &&
    window_top < sections[0].element.offset().top - 100
  ) {
